...
//        }
    ...
    template<class T>
        void Mesh<T>::merge_all(){
            while(!pool.empty()){
				...
				// 获取pool中待处理临时node对应的真实node
                Node<T>* m_node=get_node(tmp.s[2],tmp.t[2]);
				// 如果临时node不存在，则插入这个node
                if(!m_node){
                    pool.push_back(tmp);
                    insert_helper(tmp.s[2],tmp.t[2]);
                    continue;
                }
				// 若临时node与真实node 完全一致,则合并到真实node
                if(m_node->s==tmp.s&&m_node->t==tmp.t){
                    m_node->data.add(tmp.data);
                    continue;
                }
				// 若真实node为 [s0,s1,s2,s3,s4]
				// 若临时node为[a,b,c,d,e], 其中在[s0,s1,s2,s3,s4]的新节点处，插入新的node
				// 同时新的node的knot vector之后可能会变，需要先加入pool
                bool check=0;
                for(int i=0;i<5;++i){
                    if(i==2)
                        continue;
				...
				// 若没有插入任何点，设真实node为 [s0,s1,s2,s3,s4]
				// 则临时node为类似 [s0,s2,s4,a,b],[s0,s1,s2,s4,a] 这样的形式，
				// 需要按真实node中的knot加细
    ...
    template<class T>
        void Mesh<T>::adjust(Node<T>* n){
			...
			// 在4个方向的8个点上，进行 blending function refinement
			...
			// 由于[s2,t2]处的blending function 加细后分配给了 [s2,t2] 和 [s1,t2]
			// 而[s1,t2] 处的kont vector 与[t0,t1,t2,t3,t4] 不同，
			// 需要进行 violation test
			// violation1: 若[t0,t1,t2,t3,t4] 覆盖了 [s1,t2] 处的kont vector,则继续加细
			// violation2: 若[t0,t1,t2,t3,t4] 中存在 [s1,t2] 处 knot vector没有的节点，则插入新的点
            for(int i=0;i<4;++i){
                if(n->adj[i]){
                    Node<T> tmp2;
			...
			// 找出s方向上所有knot vector可能不正确的node,加入node_set
            for(map_t iter=s_map.begin();iter!=s_map.end();++iter){
                if(iter->first<n->s[1])
                    continue;
                if(iter->first>n->s[3])
			...
			//对每个blending function需要加细的node,  进行加细 
